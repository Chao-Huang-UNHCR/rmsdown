...
kobo_setup(url = "https://kobo.unhcr.org",
           token = "edab47966016256f663768f246a85a80eb30bd82")
...
uid <- filter(asset_list, name == "Results Monitoring Survey in North Fly and Bulge, Papua New Guinea") |> ## change the name accordingly
  pull(uid)
...
rm(asset,asset_list,df,S1, S2_repeat)
...
main$name_selectedfirst <- ifelse(is.na(main$name_selectedadult18), main$name_selectedadult18_2, main$name_selectedadult18)
...
ind_m <- ind %>% ###Here below add idp_valid if only you have IDPs
  select("_parent_index", "HH07_cat", "HH07_cat2", "disability",
         "citizenship", "HH07", "HH04", "HH03", "name_individual", "HH01" ) %>% 
  rename("name_respondent"="HH01",  "_index"="_parent_index") %>% 
  sjlabelled::remove_all_labels() %>% 
  rename("_index" = "X_index")
attr(main$name_respondent, "label") <- NULL
attr(main$`_index`, "label") <- NULL

main <- main %>% filter(!is.na(name_respondent)) %>% 
  left_join(.,ind_m)

rm(ind_m)
# test <- main %>% select(name_respondent,`_index`,respondent) %>% 
#   filter(!is.na(name_respondent))
# # 
# # 
# #
# test <- main %>% select(name_respondent,`_index`,respondent) %>% 
#   replace_na(list(name_individual = 0))
#   sjlabelled::remove_all_labels() %>% 
#   tibble() %>% 
#   rename(`_parent_index` = `X_index`)
# 
# test2 <- ind %>% filter(is.na(`HH01`))
# 
# 
# main2 <- left_join(test, ind_m)
# 
# rm(ind_m)
...
file_path <- "clean-data/PNG_Bulge.csv"
...
file_path <- "clean-data/PNG_Bulge_main.csv"
